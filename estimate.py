...
def estimate(test_data, model, post_activation, out_dim, batch_size, window_sliding, divisions,
    ...
    for division in divisions:
        print(f"Processing division: {division[0]} to {division[1]}")
        ...
        with torch.no_grad():
            ...
            for first in range(0, last_window-batch_sliding+1, batch_sliding):
                print(f"Processing window from {first} to {first + window_size}")
                for i in range(first, first + window_size, window_sliding):
                    ...
                    if count > checked_index:
                        print(f"{count} windows are computed.")
            ...
            for first, last in zip(range(_first, last_window, _batch_sliding),
                ...
                print(f"Processing last batch from {first} to {last}")
                # Call mini-batch data.
                x = []
        ...
        print(f"Finished processing division: {division[0]} to {division[1]}")
...
def main(options):
    ...
    # Estimate scores.
    print("Estimating anomaly scores...")
    ...
    print(f"Anomaly scores saved to {outfile}")

    
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--gpu_id", default=0, type=int)
    parser.add_argument("--dataset", default='Awing', type=str)
